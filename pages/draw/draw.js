...
Page({
    ...
    onLoad: function(option) {
        let uurrll = wx.getStorageSync('user').avatarUrl
        this.setData({
          uurrll:uurrll
        })
        this.countdown()
        ...
        let users = JSON.parse(option.users)
        let title2 = JSON.parse(option.title1)
        let roomid = JSON.parse(option.roomid)
        this.setData({
          avatarUrls:users,
          title1:title2,
          Rid: roomid
        ...
        success: function(i) {
            ...
            wx.uploadFile({
              ...
              success: (res) => {
                console.log(res)
                wx.navigateTo({
                  url: '../Guess/Guess?roomid='+JSON.stringify(that.data.Rid)+'&users='+JSON.stringify(that.data.avatarUrls) + '&ifHost='+JSON.stringify(that.data.ifHost),
                })
              },
              fail: (res) => {
